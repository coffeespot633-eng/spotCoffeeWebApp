  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setError("Gagal menyimpan data. Periksa koneksi atau konfigurasi Cloudinary.");
      console.error(err);
    } finally {
      setLoading(false);
    }
    ...
    return (
      <section className="mx-auto max-w-4xl">
        <div className="mb-6">
          <p className="text-sm font-extrabold uppercase tracking-widest text-[var(--color-coffee-500)]">Create data</p>
          <h2 className="text-3xl font-black text-[var(--color-coffee-800)]">Tambah Coffee Shop</h2>
          <p className="mt-2 text-sm text-stone-500">Isi detail tempat, fasilitas, dan foto utama untuk ditampilkan di website.</p>
        </div>
        ...
        <div className="rounded-3xl border border-[var(--color-coffee-100)] bg-white p-5 shadow-sm md:p-7">
          ...
          <form onSubmit={handleSubmit} className="grid gap-5">
            ...
            <label className="grid gap-2">
              <span className="text-sm font-bold text-[var(--color-coffee-700)]">Deskripsi dan Suasana</span>
              <textarea
                name="description"
                rows="4"
            ...
            <div>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {listFacilities.map((facility) => (
                  ...
                  >
                    <input type="checkbox" checked={selectedFacilities.includes(facility)} onChange={() => handleFacilityChange(facility)} className="sr-only" />
                    {facility}
                  </label>
                ))}
              </div>
      ...
      <label className="flex items-center gap-3 rounded-xl border border-[var(--color-coffee-200)] p-4">
        ...
        Premium
            ...
            <div className="flex flex-col-reverse gap-3 border-t border-[var(--color-coffee-100)] pt-5 sm:flex-row sm:justify-end">
              ...
              <Button type="submit" disabled={loading}>{loading ? "Mengunggah..." : "Simpan Coffee Shop"}</Button>
            </div>
          </form>
        </div>
      </section>
