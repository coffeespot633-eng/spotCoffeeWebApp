  ...
  useEffect(() => {
    const fetchDetail = async () => {
      try {
        ...
        const reviewData =
          await reviewService.getByCoffeeShop(id);
    ...
    return (
      <div className="grid min-h-[70vh] place-items-center px-4">
        <div className="text-center">
          ...
          <p className="mt-4 text-sm font-bold text-stone-500">Memuat detail tempat...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="mx-auto max-w-lg px-4 py-16 text-center">
        <div className="rounded-2xl border border-[var(--color-coffee-100)] bg-white p-8 shadow-sm">
          <p className="text-sm font-black uppercase tracking-widest text-red-600">Data tidak tersedia</p>
          <h1 className="mt-2 text-2xl font-black text-[var(--color-coffee-800)]">Coffee shop tidak ditemukan</h1>
          <p className="mt-3 text-sm leading-6 text-stone-500">{error || "Data tidak ditemukan."}</p>
...
const latestShop =
  await coffeeShopService.getById(
    shop.id
  );
...
const updatedReviews =
  await reviewService.getByCoffeeShop(
    shop.id
  );
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 md:px-8 md:py-12">
      ...
      <div className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
        <div
          className={`${glassPanel} overflow-hidden p-0`}
        >
          <div className="aspect-[16/10] bg-[var(--color-coffee-100)]">
            {shop.imageUrl ? (
              <img src={shop.imageUrl} alt={shop.name} className="
                h-full
                w-full
                object-cover
                transition
          ...
          <div className="p-6 md:p-8">
            ...
            <p className="mt-3 text-sm font-semibold text-stone-500">{shop.address}</p>
            <p className="mt-6 whitespace-pre-line text-base leading-8 text-stone-600">{shop.description}</p>
...
function Info({ label, value }) {
  return (
    <div
      ...
      "
    >
      <p className="text-xs font-extrabold uppercase tracking-widest text-[var(--color-coffee-500)]">{label}</p>
      <p className="mt-1 text-sm font-bold leading-6 text-[var(--color-coffee-800)]">{value}</p>
    </div>
  );
}
