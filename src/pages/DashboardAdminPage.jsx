...
export default function DashboardAdminPage() {
  ...
  const [latestCoffeeShops, setLatestCoffeeShops] =
  useState([]);
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        setLatestCoffeeShops(
          latestCoffeeData
        );

        const banners = bannerSnapshot.docs.map(
          (doc) => doc.data()
        );
  ...
  <section>
        ...
        return (
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <Icon
                size={22}
                className="text-[var(--color-coffee-600)]"
              />
            </div>
    ...
    <div className={`mt-6 ${glassPanel}`}>
      <SectionHeader
        subtitle="Aksi Cepat"
        title="Mulai Kelola Rekomendasi"
      />

      <p className="mt-2 max-w-2xl text-sm leading-6 text-stone-500">
        Tambahkan coffee shop baru, lengkapi fasilitas,
        upload foto, lalu cek tampilannya di halaman publik.
      </p>
  ...
  <div className={`mt-6 ${glassPanel}`}>
    <SectionHeader
      ...
      title="Coffee Shop Terbaru"
    />

    <div className="mt-4 space-y-3">
      {latestCoffeeShops.map(
        (shop) => (
          <Link
            key={shop.id}
